...
const AVATAR_COLORS = [
  "#f02424", "#f7de40", "#6cabad", "#375dde",
  "#8148dd", "#ffcfe9", "#ae7cff",
];
...
function AvatarItem({
  url, index, isSelected, bgColor, onSelect,
}: {
  url: string; index: number; isSelected: boolean; bgColor: string; onSelect: () => void;
}) {
  ...
  return (
    <button
      ...
      className={`aspect-square w-full overflow-hidden transition-all duration-200 relative ${
        isSelected ? "border-2 border-[#6ce182]" : "border border-white/10 hover:border-white/30"
      }`}
      style={{ backgroundColor: bgColor }}
      title={`Avatar ${index + 1}`}
      onClick={onSelect}
    >
      {!isLoaded && <div className="absolute inset-0 bg-eva-border/50 animate-pulse" />}
      <img
        alt={`Avatar ${index + 1}`}
...
function AddAvatarButton({
  ...
  onUpload: (file: File) => void; isUploading: boolean; uploadError: string | null;
}) {
    ...
    <div className="relative">
      ...
      <button
        ...
        className={`aspect-square w-full bg-[#080a12] border flex items-center justify-center transition-colors ${
          isUploading ? "border-[#6ce182]/50 cursor-wait"
            : uploadError ? "border-red-500 hover:border-red-400"
...
export function StepNameAvatar({
  agentName, selectedLogoUrl, customAvatars, logosData,
  isLoadingLogos, isUploading, uploadError,
  onAgentNameChange, onAvatarSelect, onAvatarUpload,
  onBack, onNext,
}: StepNameAvatarProps) {
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-3">
              {isLoadingLogos ? (
                <>
                  <div className="grid grid-cols-7 gap-3">
                    {Array.from({ length: 7 }).map((_, i) => (
                      <div key={`sk1-${i}`} className="aspect-square bg-eva-border/50 animate-pulse" />
                    ))}
                  </div>
                  <div className="grid grid-cols-7 gap-3">
                    {Array.from({ length: 7 }).map((_, i) => (
                      <div key={`sk2-${i}`} className="aspect-square bg-eva-border/50 animate-pulse" />
                    ))}
                  </div>
                </>
              ) : (
                <>
                  <div className="grid grid-cols-7 gap-3">
                    {logosData?.small?.slice(0, 7).map((url, i) => (
                      <AvatarItem
                        key={`r1-${i}`}
                        index={i}
                  ...
                  <div className="grid grid-cols-7 gap-3">
                    {logosData?.small?.slice(7, 13).map((url, i) => (
                      <AvatarItem
                        key={`r2-${i}`}
                        index={i + 7}
                        isSelected={selectedLogoUrl === url}
                        bgColor={AVATAR_COLORS[(i + 7) % AVATAR_COLORS.length]}
                        url={url}
                        onSelect={() => onAvatarSelect(url)}
                      />
                    ))}
